...
def counter_exp_finder_deri(root1, func1, root2, func2, num=200):
    ...
    for j in range(len(distance)):
        ...
        root1_minus[0] = np.clip(root1_minus[0],-1, 1)
        root1_minus[1] = np.clip(root1_minus[1],-1, 1)

        root1_plus = root1 + distance[j]
        root1_plus[0] = np.clip(root1_plus[0],-1, 1)
        root1_plus[1] = np.clip(root1_plus[1],-1, 1)

        root2_minus = root2 - distance[j]
        root2_minus[0] = np.clip(root2_minus[0],-1, 1)
        root2_minus[1] = np.clip(root2_minus[1],-1, 1)

        root2_plus = root2 + distance[j]
        root2_plus[0] = np.clip(root2_plus[0],-1, 1)
        root2_plus[1] = np.clip(root2_plus[1],-1, 1)
